...
- 第2段(技术与内容): 采用多Agent协作架构(感知/决策/执行/评估四个子Agent+协调Agent闭环), 基于Python实现了噪声估计、结构张量分析、模态识别等感知工具, 集成高斯/中值/小波/NLM/BM3D五种经典算法, 实现结构感知VT-BM3D算法与DnCNN残差卷积神经网络(PyTorch)去噪器, 配备CLAHE等增强模块、PSNR/SSIM质量评价模块、技能知识库、案例记忆库、LLM/规则双模规划器、Gradio Web界面。
- 第3段(效果): 在Set12与BSD300标准数据集上的实验表明, σ=25强噪声下DnCNN与VT-BM3D去噪效果显著优于传统滤波方法(具体数值见 experiments/results/benchmark.md, 可直接引用); 评估Agent的反思迭代机制可自动修正去噪不足; 系统离线可用, 具有良好的应用前景。

关键词建议: 智能Agent; 医学影像; 图像去噪; VT-BM3D; 卷积神经网络

## 章节映射

| 论文章节 | 对应系统内容 |
| --- | --- |
| 2 开发技术 | Python/PyTorch(DnCNN)/NumPy/scikit-image/OpenCV/BM3D库/FastAPI/Vue3+ECharts/Anthropic Claude API/多Agent架构 |
...
| 6 系统实现 | 展示前端截图(server.py + frontend/)+核心代码: vt_bm3d_denoise、DnCNN、Coordinator.process |
| 7 系统测试 | tests/test_system.py 13个用例全部通过; experiments/results/benchmark.md 性能测试(Set12+BSD300) |

## 核心代码展示建议 (2-3处)

1. `meddenoise/tools/denoise.py` 中 `vt_bm3d_denoise` — 论文核心算法
2. `meddenoise/tools/dncnn.py` 中 `DnCNN` — 深度学习去噪网络(残差学习)
3. `meddenoise/agent/coordinator.py` 中 `Coordinator.process` — Agent闭环
...
- 结论: 实现了感知-决策-执行-评估自主闭环; VT-BM3D在结构保持上优于标准BM3D, DnCNN在σ=25水平取得最佳PSNR; 双模规划器保证系统鲁棒可用
- 展望: 训练多噪声水平/盲去噪的DnCNN-B与FFDNet; 支持DICOM格式与3D体数据; 多Agent并行处理批量影像; 强化学习优化决策策略
